...
const OrderPage = () => {
  ...
  const handleCompleteOrder = async (orderId, orderItems) => {
    try {
      ...
      navigate(`/admin/billing`, { state: { orderId, orderItems } });
      
      // Optionally, refresh the order list to reflect the status change (but it's done in the billing page anyway)
      fetchOrders();
  ...
  {pendingOrders.map((order) => (
    <tr key={order._id}>
      ...
      <td>{order.status}</td>
      <td>
        <button onClick={() => handleCompleteOrder(order._id, order.items)}>Complete</button> {/* FIXED */}
       </td>
       </tr>
      ))}
      </tbody>
